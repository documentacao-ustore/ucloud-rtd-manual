...
Este documento da plataforma Ustore consiste em um manual com as principais funcionalidades do produto DCI. 
...
Para ter acesso à plataforma é necessário, anteriormente ter realizado uma primeira compra no Painel do Cliente. 
...
Clicar no botão ``Entrar``, após este procedimento, a tela inicial do portal é apresentada.
...
``inserir imagem``
 ...
 Ícone de notificação: Esse ícone é um ponto relevante para a visualização de todas as atividades que são realizadas no portal, através dele é possível observar as ações mais recentes, refletindo em forma de notificação a tela de Tarefas.


  
Configuração de conta: Possibilita acessar os detalhes da conta do usuário, onde é possível alterar o e-mail, o telefone de contato e gerenciar usuários em seu Contrato. 
Este botão ao serclicado, direciona para a página de Administração.
...
Status de Sucesso em verde significa que a operação foi concluída com sucesso.
 
  
Status de Falha em vermelho significa que ocorreu algum problema durante a operação, na própria tarefa falhada é possível constatar qual é o motivo da falha, através da coluna de Erro.

  
Status de Aprovada em verde significa que a operação ocorreu com sucesso, porém depende de alguma ação do usuário. 
Para casos como Primeira Venda, fica pendente ao usuário a confirmação do Golden Jumper de ambas as portas. 
Para casos de venda subsequente de portas, fica pendente o Golden Jumper da porta em questão.
